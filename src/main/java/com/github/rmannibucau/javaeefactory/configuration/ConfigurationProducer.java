package com.github.rmannibucau.javaeefactory.configuration;
...
public class ConfigurationProducer {
    ...
    private static final String DEFAULT_CONFIG_NAME = "javaee-factory.properties";
    ...
    private void read() {
        // classloader
        ofNullable(Thread.currentThread().getContextClassLoader().getResourceAsStream(DEFAULT_CONFIG_NAME)).ifPresent(r -> {
        ...
        // files
        Stream.of("javaeefactory.base", "openejb.base")
                .map(System::getProperty).filter(v -> v != null).map(File::new).filter(File::isDirectory)
                .map(d -> new File(d, "conf/" + DEFAULT_CONFIG_NAME)).filter(File::isFile)
                .forEach(config -> {
